...
class StrokeAnalyzer(BaseAnalyzer):
    ...
    def _calculate_stats(self) -> StrokeAnalysis:
        ...
        if avg_elbow_catch > 0:
            if ELBOW_SCORE_OPTIMAL_MIN <= avg_elbow_catch <= ELBOW_SCORE_OPTIMAL_MAX:
                ...
                ELBOW_SCORE_GOOD_MIN <= avg_elbow_catch < ELBOW_SCORE_OPTIMAL_MIN
    ...
    def draw_phase_overlay(self, frame: np.ndarray, frame_idx: int) -> np.ndarray:
        ...
        cv2.putText(frame, phase, (20, 45), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2, cv2.LINE_AA)
...
def generate_stroke_chart(analysis: StrokeAnalysis, output_path: str) -> Optional[str]:
    ...
    try:
        ...
        ax3.set_title(
            f"⚖️ Симетрія: {analysis.symmetry_score:.0f}%",
            color="white",
            fontsize=12,
            fontweight="bold",
        )
        ...
        if analysis.strokes:
            ...
            ax4.axhline(
                y=analysis.avg_stroke_duration,
                color="#00d9ff",
                linestyle="--",
                label=f"Середнє: {analysis.avg_stroke_duration:.2f}с",
            )
