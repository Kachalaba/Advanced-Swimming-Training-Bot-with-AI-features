...
SKELETON = [
    ...
    ("left_shoulder", "left_hip", "spine"),
    ("right_shoulder", "right_hip", "spine"),
    ("left_hip", "right_hip", "hip"),
    ("left_shoulder", "left_elbow", "shoulder"),
    ("left_elbow", "left_wrist", "elbow"),
    ("right_shoulder", "right_elbow", "shoulder"),
    ("right_elbow", "right_wrist", "elbow"),
    ("left_hip", "left_knee", "hip"),
    ("left_knee", "left_ankle", "knee"),
    ("right_hip", "right_knee", "hip"),
...
class BiomechanicsVisualizer:
    ...
    def process_frame(self, frame: np.ndarray, idx: int = 0, bbox: List[int] = None):
        ...
        for name, i in LANDMARKS.items():
            ...
            kps[name] = (
                int(lm.x * cw) + offset[0],
                int(lm.y * ch) + offset[1],
                lm.visibility,
            )
    ...
    def _calc_angles(self, kps: Dict) -> Dict[str, float]:
        ...
        if all(k in kps for k in ["right_shoulder", "right_elbow", "right_wrist"]):
            angles["R.elbow"] = angle_3p(
                kps["right_shoulder"][:2],
                kps["right_elbow"][:2],
                kps["right_wrist"][:2],
            )
    ...
    def _draw_trajectories(self, frame: np.ndarray) -> np.ndarray:
        colors = {
            "left_wrist": (255, 100, 100),
            "right_wrist": (100, 100, 255),
            "left_ankle": (100, 255, 100),
            "right_ankle": (255, 255, 100),
        }
        for name, pts in self.trajectories.items():
            ...
            for i in range(1, len(pts_list)):
                ...
                cv2.line(
                    ...
                    pts_list[i - 1],
                    pts_list[i],
                    tuple(int(c * alpha) for c in colors[name]),
                    max(1, int(alpha * 3)),
                    cv2.LINE_AA,
                )
        return frame
...
def visualize_biomechanics(frames: List, detections: List = None, output_dir: str = "./biomech_viz") -> Dict:
    ...
    return {
        ...
        "total": len(frames),
        "with_pose": len(valid),
        "rate": len(valid) / len(frames) if frames else 0,
        "output_dir": str(out),
    }
