...
class SplitAnalyzer:
    ...
    def detect_wall_touches(
        ...
        first_frame_with_bbox = next((d for d in detections if d["bbox"] is not None), None)
        ...
        for i in range(len(wall_touches) - 1):
            ...
            if detections and start_frame < len(detections) and end_frame < len(detections):
                ...
                if start_det.get("timestamp") is not None and end_det.get("timestamp") is not None:
                    ...
                    logger.debug(
                        f"Split {i+1}: using video timestamps ({start_det['timestamp']:.3f}s - {end_det['timestamp']:.3f}s)"
                    )
                else:
            ...
            splits.append(split)
            logger.info(f"Split {i+1}: {time_seconds:.2f}s, {speed:.2f} m/s")
