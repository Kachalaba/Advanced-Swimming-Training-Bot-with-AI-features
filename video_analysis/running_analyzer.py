...
class RunningAnalyzer(BaseAnalyzer):
    ...
    def analyze(
        self, keypoints_list: Union[List[Dict], List[List[Dict]]], fps: float = None
    ) -> Union[RunningAnalysis, MultiPersonRunningAnalysis]:
        """
    ...
    def _calculate_stats(
        self, knee_left, knee_right, forward_leans, arm_left, arm_right, total_frames
    ) -> RunningAnalysis:
    ...
    def draw_overlay(self, frame: np.ndarray, kps: Dict, analysis: RunningAnalysis = None) -> np.ndarray:
        ...
        if analysis:
            ...
            cv2.putText(
                frame,
                f"Cadence: {analysis.cadence} spm",
                (20, y),
            ...
            cv2.putText(
                frame,
                f"Knee Lift: {analysis.avg_knee_lift:.0f}°",
                (20, y),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (255, 255, 255),
            ...
            cv2.putText(
                frame,
                f"Forward Lean: {analysis.forward_lean:.1f}°",
                (20, y),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (255, 255, 255),
            ...
            cv2.putText(
                frame,
                f"Arm Symmetry: {analysis.arm_symmetry:.0f}%",
                (20, y),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (255, 255, 255),
...
def generate_running_chart(analysis: RunningAnalysis, output_path: str):
    ...
    values = [
        analysis.cadence,
        analysis.avg_knee_lift,
        analysis.arm_symmetry,
        analysis.ground_contact_ratio,
    ]
    ...
    ax4.text(0.1, 0.75, rec_text, fontsize=10, transform=ax4.transAxes, va="top", wrap=True)
