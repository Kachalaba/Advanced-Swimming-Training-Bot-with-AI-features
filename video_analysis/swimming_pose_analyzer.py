...
from video_analysis.constants import CLAHE_CLIP_LIMIT_POSE, CLAHE_TILE_GRID_SIZE, MIN_DETECTION_CONFIDENCE
...
class SwimmingPoseAnalyzer:
    ...
    def analyze_frame(
        ...
        return {
            ...
            "spine": {
                "angle": spine.total_angle,
                "deviation": spine.deviation,
                "curvature": spine.curvature,
                "points": len(spine.center_line),
            },
    ...
    def _calc_metrics(self, kps: Dict, spine: SpineAnalysis) -> SwimmingMetrics:
        ...
        if all(k in kps for k in ["left_shoulder", "left_elbow", "left_wrist"]):
            metrics.left_arm_angle = self._angle_3p(kps["left_shoulder"], kps["left_elbow"], kps["left_wrist"])
        if all(k in kps for k in ["right_shoulder", "right_elbow", "right_wrist"]):
            metrics.right_arm_angle = self._angle_3p(kps["right_shoulder"], kps["right_elbow"], kps["right_wrist"])

        # Leg angles
        if all(k in kps for k in ["left_hip", "left_knee", "left_ankle"]):
            metrics.left_leg_angle = self._angle_3p(kps["left_hip"], kps["left_knee"], kps["left_ankle"])
        if all(k in kps for k in ["right_hip", "right_knee", "right_ankle"]):
            metrics.right_leg_angle = self._angle_3p(kps["right_hip"], kps["right_knee"], kps["right_ankle"])
    ...
    def _draw_spine(self, frame: np.ndarray, keypoints: Dict, spine_data: Dict):
        ...
        if len(spine_pts) >= 2:
            ...
            cv2.line(
                frame,
                (spine_pts[0][0], start_y),
                (spine_pts[-1][0], start_y),
                (100, 100, 100),
                1,
                cv2.LINE_AA,
            )
