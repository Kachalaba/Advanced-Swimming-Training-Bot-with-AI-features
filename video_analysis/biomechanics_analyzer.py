...
from video_analysis.constants import BASE_DRAG_COEFFICIENT, CLAHE_CLIP_LIMIT, CLAHE_TILE_GRID_SIZE, WATER_DENSITY_KG_M3
...
class BiomechanicsAnalyzer:
    ...
    def _analyze_hydrodynamics(self, keypoints: Dict, angles: Dict, body_axes: Dict) -> Dict:
        ...
        if all(k in keypoints for k in ["left_shoulder", "right_shoulder", "left_hip", "right_hip"]):
            ...
            torso_height = abs(keypoints["left_shoulder"]["y"] - keypoints["left_hip"]["y"])
    ...
    def _calculate_posture_score(self, angles: Dict, body_axes: Dict, hydrodynamics: Dict) -> float:
    ...
    def _generate_recommendations(self, average_metrics: Dict, frame_analyses: List[Dict]) -> List[str]:
        ...
        if "head_elevation" in avg_angles:
            if avg_angles["head_elevation"] > 150:
                recommendations.append("⚠️ Голова слишком поднята. Опустите лицо в воду для уменьшения сопротивления.")

        # Streamline
        if avg_streamline < 70:
            recommendations.append("⚠️ Положение тела не оптимально. Старайтесь держать тело прямо и горизонтально.")
        ...
        if "left_elbow" in avg_angles and "right_elbow" in avg_angles:
            ...
            if avg_elbow > 160:
                recommendations.append("💡 Локти слишком прямые. Сгибайте локти при гребке для большей силы.")

        # Body streamline
        if "body_streamline" in avg_angles:
            deviation = abs(180 - avg_angles["body_streamline"])
            if deviation > 20:
                recommendations.append("⚠️ Тело не выровнено. Работайте над core-стабилизацией.")
    ...
    def draw_pose_on_frame(
        ...
        # 2. Середина плеч
        if (
            "left_shoulder" in keypoints
            and "right_shoulder" in keypoints
            and keypoints["left_shoulder"]["visibility"] > 0.4
            and keypoints["right_shoulder"]["visibility"] > 0.4
        ):
        ...
        # 3. Середина бёдер
        if (
            "left_hip" in keypoints
            and "right_hip" in keypoints
            and keypoints["left_hip"]["visibility"] > 0.4
            and keypoints["right_hip"]["visibility"] > 0.4
        ):
        ...
        if (
            "left_knee" in keypoints
            and "right_knee" in keypoints
            and keypoints["left_knee"]["visibility"] > 0.4
            and keypoints["right_knee"]["visibility"] > 0.4
        ):
        ...
        cv2.putText(frame, f"Posture: {score:.1f}/100", (10, y_offset), font, 0.7, color, 2)
        y_offset += 30
        ...
        for i, (label, color) in enumerate(legend_items):
            ...
            # Рисуем текст
            cv2.putText(frame, label, (x + 20, legend_y + 12), font, 0.5, (255, 255, 255), 1)
