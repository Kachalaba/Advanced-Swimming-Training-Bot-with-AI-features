...
class TrajectoryAnalyzer:
    ...
    def analyze_trajectory(
        ...
        # Overall scores
        movement_score = self._compute_movement_score(velocity_profile, streamline_quality, rhythm_metrics)

        # Generate recommendations
        recommendations = self._generate_recommendations(
            velocity_profile, body_rotation, streamline_quality, rhythm_metrics
        )
        ...
        return {
            ...
            "summary": self._create_summary(velocity_profile, streamline_quality, movement_score, len(detections)),
        }
    ...
    def _compute_velocity(self, trajectory: List[Dict], fps: float) -> Dict:
    ...
    def _compute_acceleration(self, velocity_profile: Dict, fps: float) -> Dict:
    ...
    def _detect_wall_touches(self, velocity_profile: Dict, trajectory: List[Dict]) -> Dict:
    ...
    def _compute_movement_score(self, velocity_profile: Dict, streamline_quality: Dict, rhythm_metrics: Dict) -> float:
        """Compute overall movement quality score (0-100)."""
        # Components
        velocity_consistency = 100.0 - min(velocity_profile.get("velocity_std", 0) * 2, 100.0)
        streamline_score = streamline_quality.get("avg_streamline_score", 0)
        rhythm_score = rhythm_metrics.get("rhythm_score", 0)

        # Weighted average
        movement_score = velocity_consistency * 0.3 + streamline_score * 0.4 + rhythm_score * 0.3
    ...
    def _generate_recommendations(
        ...
        if vel_std > 50:
            recommendations.append("⚠️ Неровная скорость. Работайте над плавностью движений и постоянным темпом.")
        elif vel_std < 20:
            recommendations.append("✅ Отличная стабильность скорости!")
        ...
        if streamline_score < 60:
            ...
            recommendations.append("✅ Отличная обтекаемость тела!")
        ...
        if rotation_stability < 70:
            ...
            recommendations.append("✅ Стабильное положение тела в воде!")

        # Rhythm
        rhythm_score = rhythm_metrics.get("rhythm_score", 0)
        if rhythm_score < 50:
            recommendations.append("💡 Ритм движений неравномерен. Работайте над постоянной частотой гребков.")
        elif rhythm_score > 75:
            recommendations.append("✅ Ровный ритм движений!")

        if not recommendations:
            recommendations.append("✅ Хорошая техника! Продолжайте в том же духе.")
