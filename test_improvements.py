...
async def test_contextual_help():
    ...
    try:
        ...
        if suggestion and "першого результату" in suggestion.lower() or "первого результата" in suggestion.lower():
    ...
    try:
        msg = ContextualHelp.get_motivational_message(pr_count=3, improvement_percent=5.5)
...
def test_translations():
    ...
    try:
        ...
        if "error" in uk_data and "rate_limit" in uk_data["error"]:
            print_test("Ukrainian rate_limit translation", "PASS", uk_data["error"]["rate_limit"][:50])
            passed += 1
        else:
        ...
        if "help" in uk_data and "suggestion" in uk_data["help"]:
            print_test("Ukrainian help suggestions", "PASS", f"{len(uk_data['help']['suggestion'])} suggestions")
            passed += 1
        else:
    ...
    try:
        ...
        if "error" in ru_data and "rate_limit" in ru_data["error"]:
            print_test("Russian rate_limit translation", "PASS", ru_data["error"]["rate_limit"][:50])
            passed += 1
        else:
        ...
        if "help" in ru_data and "suggestion" in ru_data["help"]:
            print_test("Russian help suggestions", "PASS", f"{len(ru_data['help']['suggestion'])} suggestions")
            passed += 1
        else:
