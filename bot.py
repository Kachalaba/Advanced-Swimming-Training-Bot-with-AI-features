...
def setup_dispatcher(
    ...
    from handlers.onboarding_tour import router as onboarding_tour_router
    ...
    dp.message.middleware(RateLimitMiddleware(rate=10, per=60))  # 10 messages per minute
    dp.message.middleware(CommandRateLimitMiddleware(rate=5, per=60))  # 5 commands per minute
    ...
    dp.include_router(onboarding_tour_router)  # Interactive tour for new users
...
async def main() -> None:
    ...
    # Create a simple container-like object for healthcheck
    class SimpleContainer:
        def __init__(self, bot, role_service):
            self.bot = bot
            self.role_service = role_service
    
    container = SimpleContainer(bot, role_service)
