...
async def process_video(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        video_file = await message.bot.download(
            message.video.file_id, destination=temp_dir / "input_video.mp4"
        )
        ...
        if Path(report_files["chart"]).exists():
            ...
            await message.answer_photo(
                chart_file, caption="📈 График твоей скорости по отрезкам"
            )
        ...
        if Path(report_files["pdf"]).exists():
            ...
            await message.answer_document(
                pdf_file, caption="📄 Полный отчёт с анализом"
            )
        ...
        if detection_result["detected_images"]:
            ...
            if Path(sample_image).exists():
                ...
                await message.answer_photo(
                    img_file, caption="🎯 Пример детекции пловца"
                )
