...
from utils.contextual_help import ContextualHelp, format_suggestion_message
...
async def _send_menu(
    ...
    # Add contextual help suggestion if available
    # TODO: Get actual user stats from stats_service
    suggestion = await ContextualHelp.get_suggestion(
        user_id=message.from_user.id,
        total_results=0,  # Will be populated from stats_service
        days_since_last_result=999,
        last_progress_check_days=999,
        unchecked_prs=0,
        has_completed_onboarding=True,
    )

    if suggestion:
        full_text += format_suggestion_message(suggestion)

    keyboard = build_modern_main_menu(role, context.as_dict())
